...
Meeting dates –
We plan to meet every Tuesday afternoon at 1 pm. Location either at James Watt Lounge Area or at GRID.
...
Most of the variables and functions have meaningful names that aid the understanding of their functionality. Most of the variables are local instead of global. Variables are only global when they need to be.

Code validation/testing plan (automated vs user) –
No Unit testing code plan available but bugs and features were agile tested during the development of the game

Manage code changes/versions (different file types/bugs/tasks) –
different versions of the game are accessible from the GIT repository. Older versions of the game can be pulled.
...
Plans/Objectives –
Week 1 – All members present at the meeting at James Watt. Meeting used to brainstorm ideas about what our game should be. Ideas discussed Snake Multiplayer and Tron Multiplayer
Week 2 – All members present at the meeting at James Watt. Finalized that game idea going forward is Snake Multiplayer. Spoke about what roles each member will have in the creation of the game. Basically, who is handling what.
...
Week 3 – All members present at the meeting at GRID. Joe and Eniolubi made basic HTML, CSS, and JavaScript for the client-side of the game. They implemented a basic index page and linked it to a JS script. Index page includes user play button and login card. CSS page styles the index page. JS script sends users login information to the server and animates buttons Chris had made hardcoded server IP. Cameron has started the basic Database with queries.

Week 4 – All members present at meeting at GRID. Joe has made a JS script that sends clients login details to the server. Eniolubi has written CSS, Javascript and HTML for a leaderboard and the script to let it fade in. Chris
cleaned up syntax errors and connected the server to Cameron’s external database.

Week 5 – All members present at the meeting at James Watt. Meeting to go over what we have done so far (like the implementation of the server (main goal)) and what we still have to do (features that need to be implemented like the collision of characters etc.)

Week 6 – All members present at the meeting at James Watt. Continuing to go over what needs to be done still for this game. Ideas brought up by Joseph was register div so users can create new accounts before they play the game.
Chris reminded members that the mobile version of the game has to be implemented (although further down the line of progress) along with optimizing resizing of the game. Eniolubi suggested to Cameron that queries for the leaderboard have to be created.

Week 7 – All members present at meeting at GRID. Catch up of what each member has done so far. members noticed that no commits have been pushed to the GIT repository in a while.

Week 8 – All members present at the meeting in EM2.50. Discussed progress so far and what work needed to be completed, we also decided to change the game mechanics slightly from Snake to Tron, we decided that we could make this a more polished game as snake was proving harder than expected.

Week 9 – All members present at the meeting at James Watt Centre. Looked over work done by Chris on the server monitor and handed out jobs for the week. Meeting 2 in GRID, all members present, started working on merging the server monitor with the client, adding some functionality both serverside and clientside.
...
Week 10 – All members present at meetings throughout the week. This week was spent finalizing the client aesthetics, merging different versions of the client into one and implementing mobile compatibility which was done by Joe, Eniolubi, and Cameron, while Chris worked on serverside improvements and optimization, and helping to fix bugs both serverside and clientside.  
...
Controls are sent as vector components and sent to the server to be processed, the vector is then normalized and processed using delta-time calculations.
This stops any form of manipulation attack.

Interaction involvements/collisions –
Collision is done server-side so as to avoid cheating.  Each time a player requests a new movement, the new body segment is checked against existing segments
...
Testing:
Usability –
For the mobile version, users will be given LEFT-RIGHT-UP-DOWN div created buttons which when pressed will be used to alter the direction of their sprite in-game. This aids usability as buttons are simplistic
in functionality and are positioned for easy use and understanding.
Gameplay even though straight forward enough contains a short sentence that explains how to play the game, further aiding usability.
...
The information that is sent to/from the server is the users’ login request, successful login, the client’s coordinates, and the server’s coordinates. And it keeps looping.
...
Game is optimized to be fullscreen for different browsers such as:
-	Google Chrome
-	Fire Fox
...
So far there is one external asset. It is a .png file that is used as our game’s background. Our coursework doesn’t contain any game sounds either.
...
Bugs, features, releases are regularly updated -
